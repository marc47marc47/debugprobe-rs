//! STM32F401CCU6 "Black Pill"（WeAct）layer-2 測試韌體：板載 LED 閃爍 + 外接 OLED 狀態 + UART 雙向 rx/tx。
//! 對等於 RP2040 的 `src/bin/uartmon.rs`（OLED+LED+RX echo）＋ `uarthello.rs`（週期 TX 心跳），
//! 用來驗證 debugprobe-rs 探針可把 STM32F401 當作 layer 2 目標（SWD 燒錄/重置 + UART 橋接）。
//!
//! 接線（Black Pill 無板載偵錯器；自身 USB-C 供電；探針共地）：
...
//! - OLED（SSD1306 I2C1）：SCL=PB8、SDA=PB9、VCC=3V3、GND
//! - LED：PC13（板載，active-low，免接線）
//!
//! 建置：cd stm32f401-target && cargo build --release
//! 燒錄：probe-rs download --chip STM32F401CCUx --probe 2e8a:000c-0:<serial> --protocol swd --speed 1000 \
//!         target/thumbv7em-none-eabihf/release/stm32f401-target
//! 注意：出廠 Black Pill 多半開著 RDP Level 1，需先用 OpenOCD `stm32f2x unlock 0` 清讀保護（會 mass erase）。
...
async fn main(_spawner: Spawner) {
    ...
    draw("f401 layer2", "USART1 PA9/PA10", "waiting...");
    ...
    // 板載 LED（PC13，Black Pill，active-low）。
    ...
    loop {
        ...
        match select(uart.read(&mut buf), Timer::after(Duration::from_millis(1000))).await {
            Either::First(Ok(len)) if len > 0 => {
                ...
                draw("f401 layer2", last_line.as_str(), l3.as_str());
            }
            Either::Second(()) => {
                let mut line: heapless::String<48> = heapless::String::new();
                let _ = write!(line, "hello from f401 #{}\r\n", tx_n);
