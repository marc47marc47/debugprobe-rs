//! STM32F103C8 "Blue Pill" layer-2 測試韌體：板載 LED 閃爍 + 外接 OLED 狀態 + UART 雙向 rx/tx。
//! 與 `stm32f401-target` 功能/腳位一致（只差晶片：F103 為 Cortex-M3 無 FPU），
//! 用來驗證 debugprobe-rs 探針可把 STM32F103 當作 layer 2 目標（SWD 燒錄/重置 + UART 橋接）。
//!
//! 接線（Blue Pill 無板載偵錯器；自身 USB/3V3 供電；探針共地；腳位同 F401）：
//! - SWD（4-pin 排針）：探針 A.GP2→SWCLK(PA14)、A.GP3→SWDIO(PA13)、GND；A.GP1→NRST（解 RDP/喚醒用）
//! - UART：A.GP4(UART1 TX)→PA10(USART1 RX)、A.GP5(UART1 RX)←PA9(USART1 TX)、GND
//! - OLED（SSD1306 I2C1）：SCL=PB8、SDA=PB9、VCC=3V3、GND（F103 上 PB8/PB9 走 I2C1 重映射，embassy 自動設定）
//! - LED：PC13（板載，active-low，免接線）
//!
//! 建置：cd stm32f103-target && cargo build --release
//! 燒錄：probe-rs download --chip STM32F103C8Tx --probe 2e8a:000c-0:<serial> --protocol swd --speed 1000 \
//!         target/thumbv7m-none-eabi/release/stm32f103-target
//! 注意：若 Blue Pill 開著 RDP，需先用 OpenOCD `stm32f1x unlock 0` 清讀保護（會 mass erase）。
...
async fn main(_spawner: Spawner) {
    ...
    // OLED（I2C1：SCL=PB8, SDA=PB9，F103 走 I2C1 重映射）；400kHz。
    // F103 的 I2C(v1) 無內部上拉設定（不同於 F4 v2），靠 OLED 模組自帶的上拉電阻。
    ...
    draw("f103 layer2", "USART1 PA9/PA10", "waiting...");
    ...
    // 板載 LED（PC13，Blue Pill，active-low）。
    ...
    loop {
        ...
        match select(uart.read(&mut buf), Timer::after(Duration::from_millis(1000))).await {
            Either::First(Ok(len)) if len > 0 => {
                ...
                draw("f103 layer2", last_line.as_str(), l3.as_str());
            }
            Either::Second(()) => {
                let mut line: heapless::String<48> = heapless::String::new();
                let _ = write!(line, "hello from f103 #{}\r\n", tx_n);
